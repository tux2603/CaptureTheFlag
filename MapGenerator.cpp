...
int TerritoryAllocationCell::getOwner()
{
  ...
  for (int i = 0; i < numTeams; ++i)
  {
    if (claims[i] >= MIN_CLAIM_TO_OWN && owner == NO_OWNER)
      owner = i;
    else if (claims[i] >= MIN_CLAIM_TO_OWN && owner != NO_OWNER)
      return DISPUTED;
  }
...
void TerritoryAllocator::fillMap(TerrainMap &map)
{
  this->fillRawMap(map.tiles, map.territoryMask, map.width, map.height, map.numTeams);
}

RandomBlobbyTerritoryAllocator::RandomBlobbyTerritoryAllocator() : RandomBlobbyTerritoryAllocator(0.02) {}
RandomBlobbyTerritoryAllocator::RandomBlobbyTerritoryAllocator(double waterRatio) : waterRatio(waterRatio) {}

void RandomBlobbyTerritoryAllocator::fillRawMap(TerrainType **tiles, int **territoryMask, int width, int height, int numTeams)
{
#ifdef DEBUG
        ...
        if (cellOwner >= 0)
        {
          ...
          for (int i = 0; i < 8; ++i)
          {
            ...
            if (adjacentCellOwner != cellOwner)
            {
              ...
              if (currentAdjacentCells[i]->terrain == TerrainType::Brambles)
                nextAdjacentCells[i]->claims[cellOwner] += 1;
              else if (currentAdjacentCells[i]->terrain == TerrainType::Forest)
                nextAdjacentCells[i]->claims[cellOwner] += 2;
              else if (currentAdjacentCells[i]->terrain == TerrainType::Hills)
                nextAdjacentCells[i]->claims[cellOwner] += 2;
              else if (currentAdjacentCells[i]->terrain == TerrainType::Field)
                nextAdjacentCells[i]->claims[cellOwner] += 4;
              ...
              //   nextAdjacentCells[i]->claims[cellOwner] += (long)(nextAdjacentCells[i]->claims[cellOwner] * 2);
              // make sure we haven't overflowed long
              if (currentAdjacentCells[i]->claims[cellOwner] < 0)
                nextAdjacentCells[i]->claims[cellOwner] = TerritoryAllocationCell::MIN_CLAIM_TO_OWN;
            }
          }
...
SmartWedgeTerritoryAllocator::SmartWedgeTerritoryAllocator() : SmartWedgeTerritoryAllocator(0.02) {}
SmartWedgeTerritoryAllocator::SmartWedgeTerritoryAllocator(double waterRatio) : waterRatio(waterRatio) {}

// TODO have SmartWedgeTerritoryAllocator and RandomBlobbyTerritoryAllocator share a super class
// ! DRY violation
void SmartWedgeTerritoryAllocator::fillRawMap(TerrainType **tiles, int **territoryMask, int width, int height, int numTeams)
{
  // Create two arrays to store territory allocations
  TerritoryAllocationCell ***territoryCellsA = new TerritoryAllocationCell **[height];
  TerritoryAllocationCell ***territoryCellsB = new TerritoryAllocationCell **[height];
  ...
  // Get the number of cells around the outline of the map
  int perimeter = width * 2 + height * 2 - 4;

  // Thingies to generate random numbers
  default_random_engine g(time(0));
  uniform_int_distribution<int> randStart(0, perimeter - 1);

  // Get a random starting location around the perimeter
  double nextPrison = randStart(g);

  double prisonSpacing = perimeter / (numTeams - 1);
  ...
  for (int i = 0; i < numTeams; ++i)
  {
    nextPrison = fmod(nextPrison, perimeter);

    // If we are on the first edge
    if (nextPrison < width)
    {
      x = (int)nextPrison;
      y = 0;
    ...
    // If it is on the second edge
    else if (nextPrison < height + width - 1)
    {
      x = width - 1;
      y = (int)nextPrison - (width - 1);
    }

    else if (nextPrison < height + width * 2 - 2)
    {
      x = width * 2 + height - 3 - (int)nextPrison;
      y = height - 1;
    }
    ...
    {
      x = 0;
      y = width * 2 + height * 2 - 4 - (int)nextPrison;
    }

    nextPrison += prisonSpacing;
    // Once a suitable candidate is found, assign the owner at that location to be team i
    (*currentTerritory)[y][x]->claims[i] = TerritoryAllocationCell::MIN_CLAIM_TO_OWN;
    (*nextTerritory)[y][x]->claims[i] = TerritoryAllocationCell::MIN_CLAIM_TO_OWN;

    // Make this cell team i's prison
    tiles[y][x] = TerrainType::Prison;
  }
      ...
      for (int y = 0; y < height; ++y)
      {
        ...
        if (cellOwner >= 0)
        {
...
#ifdef DEBUG
          //cout << "Wrap around is set to " << wrapAround << endl;
#endif // DEBUG

          bool *skipCell = new bool[8];
          skipCell[0] = !wrapAround && (y >= height - 1 || x <= 0);
          skipCell[1] = !wrapAround && y >= height - 1;
          skipCell[2] = !wrapAround && (y >= height - 1 || x >= width - 1);
          skipCell[3] = !wrapAround && x >= width - 1;
          skipCell[4] = !wrapAround && x <= 0;
          skipCell[5] = !wrapAround && (y <= 0 || x <= 0);
          skipCell[6] = !wrapAround && y <= 0;
          skipCell[7] = !wrapAround && (y <= 0 || x >= width - 1);
          ...
          for (int i = 0; i < 8; ++i)
          {
            if(skipCell[i]) {
#ifdef DEBUG
              //cout << "Skipping cell requiring wrap around" << endl;
#endif // DEBUG
              continue;
            }
...
// ###########################################################################
// #####                  MAP GENERATOR IMPLEMENTATIONS                  #####
// ###########################################################################

void MapGenerator::fillMap(TerrainMap &map)
{
  this->fillRawMap(map.tiles, map.width, map.height, map.numTeams);
}

BarMapGenerator::BarMapGenerator() {}
BarMapGenerator::BarMapGenerator(const BarMapGenerator &toCopy) {}

void BarMapGenerator::fillRawMap(TerrainType **tiles, int width, int height, int numTeams)
{
  ...
  for (int x = 0; x < width; x++)
  {
    for (int y = 0; y < height; y++)
    {
      ...
      // TODO Assign the team at that tile
      // TODO territoryMask[y][x] = (int)floor(x / territorySpacing);
...
PolygonMapGenerator::PolygonMapGenerator(const PolygonMapGenerator &toCopy) : PolygonMapGenerator(toCopy.startPolygonRatio, toCopy.endPolygonRatio) {}

PolygonMapGenerator::PolygonMapGenerator(double startPolygonRatio, double endPolygonRatio) : startPolygonRatio(startPolygonRatio), endPolygonRatio(endPolygonRatio) {}
...
void PolygonMapGenerator::fillRawMap(TerrainType **tiles, int width, int height, int numTeams)
{
  ...
  cout << "Area of the map is " << area << ". The generator will start with " << startPolygons << " polygons and end with " << endPolygons << " polygons." << endl;
  ...
  for (int i = 0; i < startPolygons; ++i)
  {
    ...
    if (randT < 0.60)
      randomTerrain = TerrainType::Field;
    else if (randT < 0.80)
      randomTerrain = TerrainType::Forest;
    else if (randT < 0.95)
      randomTerrain = TerrainType::Hills;
    else
      randomTerrain = TerrainType::Brambles;
