...
// ###########################################################################
// #####             TERRITORY ALLOCATOR CLASSES DECLARATIONS            #####
// ###########################################################################
...
class RandomBlobbyTerritoryAllocator : public TerritoryAllocator
{
public:
  double waterRatio;
  RandomBlobbyTerritoryAllocator();
  RandomBlobbyTerritoryAllocator(double waterRatio);
  void fillRawMap(TerrainType **tiles, int **territoryMask, int width, int height, int numTeams);
};

class SmartWedgeTerritoryAllocator : public TerritoryAllocator
{
public:
  double waterRatio;
  SmartWedgeTerritoryAllocator();
  SmartWedgeTerritoryAllocator(double waterRatio);
  void fillRawMap(TerrainType **tiles, int **territoryMask, int width, int height, int numTeams);
};

// ###########################################################################
// #####               MAP GENERATOR CLASSES DECLARATIONS                #####
// ###########################################################################

class MapGenerator
{
public:
  virtual void fillRawMap(TerrainType **tiles, int width, int height, int numTeams) = 0;
  void fillMap(TerrainMap &map);
};
...
public:
  ...
  void fillRawMap(TerrainType **tiles, int width, int height, int numTeams);
};
